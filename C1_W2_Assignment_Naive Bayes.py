...
#     V = None    
    ...
    D_pos = sum(train_y) #None

    # Calculate D_neg, the number of negative documents
    D_neg = D- D_pos #None

    # Calculate logprior
    logprior = np.log(D_pos / D_neg) #None
...
def test_naive_bayes(test_x, test_y, logprior, loglikelihood, naive_bayes_predict=naive_bayes_predict):
    ...
    #error = None
    correct_predictions = sum([1 for predicted, actual in zip(y_hats, test_y) if predicted == actual])
...
def get_ratio(freqs, word):
    ...
    pos_neg_ratio['ratio'] = (pos_neg_ratio['positive'] + 1) / (pos_neg_ratio['negative'] + 1)#(None) / (None)
    ### END CODE HERE ###
...
def get_words_by_threshold(freqs, label, threshold, get_ratio=get_ratio):
    ...
    for key in freqs.keys():
        word, _ = key # Extract the word from the (word, label) pair  #None
        ...
        if label == 1 and pos_neg_ratio['ratio'] >= threshold:
            ...
            word_list[word] = pos_neg_ratio #None
